    ...
    header: ({ column }) => {
      ...
      return (
        <Button variant="ghost" onClick={() => column.toggleSorting(sort === "asc")}>
          Supplier Id
          {!sort ? <ArrowUpDown className="size-4" /> : sort === "asc" ? <ArrowUp className="size-4" /> : <ArrowDown className="size-4" />}
        </Button>
      );
    },
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="text-center font-medium">
        {formatCurrency(row.getValue("openingBalance") as number)}
      </div>
    ),
  },
  {
    ...
    cell: ({ row }) => (
      <div className="text-center font-medium">
        {formatCurrency(row.getValue("purchaseDue") as number)}
      </div>
    ),
  },
  {
    ...
    cell: ({ row }) => (
      <div className="text-center font-medium">
        {formatCurrency(row.getValue("purchaseReturnDue") as number)}
      </div>
    ),
  },
  {
    ...
    cell: ({ row, table }) => {
      ...
      return <SupplierActions supplier={row.original} branches={meta?.branches || []} />;
    },
  },
];
...
export const SupplierActions = ({ supplier, branches }: SupplierActionsProps) => {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <Button variant="ghost" size="sm" onClick={() => setOpenPayment(true)} className="h-8 w-8 p-0">
        <Clock1 className="h-4 w-4" />
      </Button>
      <Button variant="ghost" size="sm" onClick={() => setOpenEdit(true)} className="h-8 w-8 p-0">
        <Edit2 className="h-4 w-4" />
      </Button>
      <Button variant="ghost" size="sm" onClick={() => setOpenDelete(true)} className="text-destructive hover:text-destructive h-8 w-8 p-0">
      ...
      <SupplierDeleteDialog supplier={supplier} open={openDelete} setOpen={setOpenDelete} />
    </div>
  );
};