...
import {
    ArrowLeft, ShoppingBag, RotateCcw, Wallet, Star,
    TrendingUp, AlertCircle, Phone, Mail, MapPin,
    ChevronDown, ChevronRight, CreditCard, IndianRupee,
    Award, Crown, Shield, Zap, Calendar, Package,
...
function SaleRow({ sale }: { sale: Sale }) {
    ...
    const status = (sale.paymentStatus?.toLowerCase() ?? "due") as keyof typeof STATUS_CONFIG;
        ...
        <div style={{ border: "0.5px solid var(--color-border-tertiary)", borderRadius: 10, overflow: "hidden", marginBottom: 8 }}>
            ...
            >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                        ...
                        {sale.branch && <span style={{ fontSize: 10, color: "var(--color-text-secondary)", background: "var(--color-background-secondary)", padding: "1px 6px", borderRadius: 4 }}>{sale.branch.name}</span>}
                    </div>
            ...
            {open && (
                <div style={{ padding: "0 16px 14px", borderTop: "0.5px solid var(--color-border-tertiary)", background: "var(--color-background-secondary)" }}>
                    ...
                    {sale.paymentDue > 0 && (
                        <div style={{ marginTop: 10, padding: "6px 10px", background: "#fef2f2", borderRadius: 6, fontSize: 12, color: "#dc2626", fontWeight: 500 }}>
                            ⚠ Balance due: {fmt(sale.paymentDue)}
                        </div>
                    )}
                </div>
            )}
...
function ReturnRow({ ret }: { ret: SalesReturn }) {
    ...
    return (
        <div style={{ border: "0.5px solid #fecaca", borderRadius: 10, overflow: "hidden", marginBottom: 8 }}>
            <div
                onClick={() => setOpen(!open)}
                style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", cursor: "pointer", background: open ? "#fef2f2" : "var(--color-background-primary)", transition: "background 0.1s" }}
            >
                <div style={{ color: "#dc2626", transition: "transform 0.15s", transform: open ? "rotate(90deg)" : "rotate(0deg)" }}>
                    <ChevronRight size={14} />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                        ...
                        {ret.branch && <span style={{ fontSize: 10, color: "#dc2626", background: "#fef2f2", padding: "1px 6px", borderRadius: 4 }}>{ret.branch.name}</span>}
                    </div>
                    <div style={{ fontSize: 12, color: "var(--color-text-secondary)", marginTop: 2 }}>
...
export function CustomerHistoryClient({
    customer, sales, returns, balancePayments,
    loyaltyPoints, loyaltyTier, totalSpent, totalReturned,
}: Props) {
    const router = useRouter();
    const [tab, setTab] = useState<"purchases" | "returns" | "balance" | "rewards">("purchases");
    ...
    const progressPct = tier.needed
        ? Math.min(100, (loyaltyPoints / tier.needed) * 100)
        : 100;

    const totalDue = customer.salesDue + (customer.openingBalance ?? 0);
    ...
    const TABS = [
        ...
        { id: "balance" as const, label: "Balance", icon: <Wallet size={13} />, count: balancePayments.length },
...
<div style={{ display: "flex", gap: 8, flexWrap: "wrap", flexShrink: 0 }}>
                    {[
                        ...
                        { label: "Purchases", value: sales.length, color: "#16a34a" },
                        { label: "Balance Due", value: fmt(totalDue), color: totalDue > 0 ? "#dc2626" : "#16a34a" },
            ...
            <div style={{ display: "flex", gap: 4, background: "var(--color-background-secondary)", borderRadius: 10, padding: 4, marginBottom: 20, border: "0.5px solid var(--color-border-tertiary)" }}>
                {TABS.map((t) => (
                    <button
                        ...
                        onClick={() => setTab(t.id)}
                        style={{
                            flex: 1, padding: "8px 10px", border: tab === t.id ? "0.5px solid var(--color-border-secondary)" : "none",
                            ...
                            display: "flex", alignItems: "center", justifyContent: "center", gap: 6, transition: "all 0.1s",
                        }}
                    >
                        {t.icon}
                        <span className="hidden sm:inline">{t.label}</span>
                        <span style={{ background: tab === t.id ? "#7F77DD" : "var(--color-border-secondary)", color: tab === t.id ? "white" : "var(--color-text-secondary)", borderRadius: 20, padding: "0px 6px", fontSize: 10, fontWeight: 700, minWidth: 18, textAlign: "center" }}>
                            {t.count}
                        </span>
                    </button>
                ))}
            ...
            {tab === "balance" && (
                <div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12, marginBottom: 20 }}>
                        {[
                            { label: "Opening Balance", value: fmt(customer.openingBalance ?? 0), color: "#555", icon: <Wallet size={16} /> },
                            { label: "Sales Due", value: fmt(customer.salesDue ?? 0), color: "#dc2626", icon: <AlertCircle size={16} /> },
                            { label: "Return Due", value: fmt(customer.salesReturnDue ?? 0), color: "#d97706", icon: <RotateCcw size={16} /> },
                            { label: "Net Balance", value: fmt(totalDue), color: totalDue > 0 ? "#dc2626" : "#16a34a", icon: <TrendingUp size={16} /> },
                        ].map((card) => (
                            <div key={card.label} style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-secondary)", borderRadius: 10, padding: "14px 16px" }}>
                                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, color: card.color }}>{card.icon}</div>
                    ...
                    {balancePayments.length === 0 ? (
                        <EmptyState icon={<CreditCard size={32} />} message="No payment records" />
                    ) : (
                        balancePayments.map((pay) => (
                            <div key={pay.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 10, marginBottom: 8, background: "var(--color-background-primary)" }}>
                                <div>
                                    <div style={{ fontWeight: 600, fontSize: 13 }}>{fmt(pay.amount)}</div>
                                    <div style={{ fontSize: 11, color: "var(--color-text-secondary)", marginTop: 2 }}>{pay.paymentMethod} • {fmtDate(pay.paidOn)}</div>
                                    {pay.paymentNote && <div style={{ fontSize: 11, color: "var(--color-text-secondary)", marginTop: 1 }}>{pay.paymentNote}</div>}
                                </div>
                                <div style={{ background: "#f0fdf4", color: "#16a34a", borderRadius: 20, padding: "3px 10px", fontSize: 11, fontWeight: 600 }}>
                                    Paid
                                </div>
                            </div>
                        ))
            ...
            {tab === "rewards" && (
                <div>
                    ...
                    {/* How points work */}
                    <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-secondary)", borderRadius: 12, padding: "16px 20px", marginBottom: 20 }}>
                        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 12 }}>How Points Work</div>
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12 }}>
                            {[
                                { label: "1 pt per ₹100 spent", icon: <ShoppingBag size={14} />, color: "#7F77DD" },
                                { label: "Points deducted on return", icon: <RotateCcw size={14} />, color: "#dc2626" },
                                { label: "Silver at 200 pts", icon: <Star size={14} />, color: "#9e9e9e" },
                                { label: "Gold at 500 pts", icon: <Award size={14} />, color: "#f5a623" },
                                { label: "Platinum at 1000 pts", icon: <Crown size={14} />, color: "#7F77DD" },
                            ].map((rule) => (
                                <div key={rule.label} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: "var(--color-text-secondary)" }}>
                                    <span style={{ color: rule.color, flexShrink: 0 }}>{rule.icon}</span>
                                    {rule.label}
                                </div>
                            ))}
                        </div>
                    ...
                    {/* Points breakdown */}
                    <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 10 }}>Points Breakdown</div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
                        {[
                            ...
                            { label: "Total purchases value", value: fmt(totalSpent), color: "#7F77DD" },
                        ].map((row) => (
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                        {(
                            [
                                ...
                                { tier: "Gold", pts: "500+", perks: ["10% discount on all purchases", "Dedicated relationship manager", "Free delivery"], color: "#f5a623" },
                                { tier: "Platinum", pts: "1000+", perks: ["15% discount on all purchases", "Exclusive preview events", "Free alterations", "VIP lounge access"], color: "#7F77DD" },
                            ] as const
                        ).map((t) => {
                            const isCurrentOrBelow =
                                (loyaltyTier === "Bronze") ||
                                (loyaltyTier === "Silver" && (t.tier === "Bronze" || t.tier === "Silver")) ||
                                (loyaltyTier === "Gold" && t.tier !== "Platinum") ||
                                (loyaltyTier === "Platinum");
                            const isCurrent = t.tier === loyaltyTier;

                            return (
                                <div key={t.tier} style={{ border: isCurrent ? `2px solid ${t.color}` : "0.5px solid var(--color-border-tertiary)", borderRadius: 10, padding: "12px 16px", background: isCurrent ? `${t.color}08` : "var(--color-background-primary)", opacity: isCurrentOrBelow ? 1 : 0.5 }}>
                                    ...
                                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                                        {t.perks.map((perk) => (
                                            <span key={perk} style={{ fontSize: 11, color: isCurrentOrBelow ? "var(--color-text-primary)" : "var(--color-text-secondary)", background: "var(--color-background-secondary)", padding: "3px 8px", borderRadius: 20, display: "flex", alignItems: "center", gap: 4 }}>
                                                {isCurrentOrBelow ? "✓" : "🔒"} {perk}
                                            </span>
                                        ))}
                                    </div>
                                </div>
