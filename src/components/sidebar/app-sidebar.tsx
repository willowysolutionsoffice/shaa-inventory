...
export function AppSidebar({ user, ...props }: AppSidebarProps) {
  ...
  const visibleNavMain = SIDEBAR_DATA.navMain
    .filter((group) => isGroupVisible(group, userPermissions))
    .map((group) => {
      const kids = visibleChildren(group, userPermissions);
      return { ...group, children: kids.length > 0 ? kids : undefined };
    });

  const visibleAdmin = SIDEBAR_DATA.admin
    .filter((group) => isGroupVisible(group, userPermissions))
    .map((group) => {
      const kids = visibleChildren(group, userPermissions);
      return { ...group, children: kids.length > 0 ? kids : undefined };
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-1/2 -right-3 z-50 hidden md:flex -translate-y-1/2">
        <button
          ...
          className="
            ...
            hover:scale-105 hover:bg-slate-50 dark:hover:bg-slate-700
          "
        >
          {state === "expanded" ? (
      ...
      <SidebarHeader className="
        ...
        px-3 py-4 shrink-0 transition-all duration-300
      ">
      ...
      <SidebarContent className="
        ...
        bg-gradient-to-b from-[#4F7DF3] via-[#4672E8] to-[#3B66D9]
        dark:from-slate-900 dark:via-slate-900 dark:to-slate-900
        text-white overflow-y-auto overflow-x-hidden no-scrollbar py-4
      ">
        ...
        {visibleAdmin.length > 0 && (
          <div className="mt-4 shrink-0 group-data-[collapsible=icon]:hidden">
            <div className="px-5 mb-2 text-[10px] font-semibold uppercase tracking-[0.22em] text-white/50">
              {isAdmin ? "Admin & Operations" : "Management"}
            </div>
            <div className="px-2">
      ...
      <SidebarFooter className="
        ...
        text-white p-2 shrink-0 transition-all duration-300
      ">
