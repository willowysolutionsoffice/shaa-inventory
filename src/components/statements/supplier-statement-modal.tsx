...
export function SupplierStatementModal({
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            setLoading(true);
            try {
                const result = await getStatementData({ id: supplierId, type: "supplier" });
                if (result?.data) {
    ...
    const statementItems = useMemo(() => {
        ...
        return data.transactions.map((tx) => {
            // For Supplier: Balance = Opening + Credit (Purchases) - Debit (Payments/Returns)
            ...
            <DialogContent className="max-w-5xl max-h-[90vh] flex flex-col">
                <DialogHeader>
                    <DialogTitle className="flex justify-between items-center pr-8">
                        <span>Supplier History Statement - {data?.name || "Loading..."}</span>
                        {data && (
                            <ExportPdfButton
                                data={statementItems}
                                title="Supplier Ledger Statement"
                                subtitle={`Supplier: ${data.name}`}
                                openingBalance={data.openingBalance}
                                type="supplier"
                                filename={`Supplier-Statement-${data.name?.replace(/\s+/g, '-')}`}
                            />
                        )}
                    </DialogTitle>
                </DialogHeader>
                ...
                <div className="flex-1 overflow-auto mt-4 px-1">
                    {loading ? (
                        <div className="flex justify-center items-center h-64">
                            <IconLoader2 className="animate-spin h-8 w-8 text-orange-600" />
                        </div>
                    ) : (
                        <Table>
                            ...
                            {statementItems.length > 0 && (
                                <TableFooter className="bg-muted/50">
                                    <TableRow>
                                        <TableCell colSpan={4} className="text-right font-bold text-lg">Current Total Payable</TableCell>
