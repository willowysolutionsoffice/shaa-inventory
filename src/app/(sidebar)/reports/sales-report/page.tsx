...
export default function SalesReportPage() {
  ...
  const fetchReport = useCallback(async (f: Filters) => {
    ...
    try {
      ...
      const sales: Sale[] = payload?.sales ?? payload ?? [];

      // Build product→meta map from whatever the API returns in item.product
      ...
      // Flatten sales → rows (brand/supplier filter happens client-side below)
