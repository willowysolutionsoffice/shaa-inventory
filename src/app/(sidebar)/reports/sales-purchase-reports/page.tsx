...
import { getPurchaseReport } from "@/actions/purchase-actions"; // ← server action
...
const DEFAULT_FILTERS: Filters = {
    ...
    brandId: "", subBrandId: "", branchId: "", supplierId: "",
};
...
function exportToCSV(
    ...
    const dataRows = rows.map((r) =>
        [r.slNo, r.poNo, r.date, r.year, r.month,
        r.itemCode, r.itemName, r.qty, r.baseUnit,
        r.unitPrice, r.amount,
        r.supplierName, r.branchName, r.paymentStatus].map(esc).join(",")
    );
    const totalsRow = [
        "Total", "", "", "", "", "", "",
        totals.qty, "",
...
function exportToPDF(
    ...
    <tr>
      ...
      <td>${r.supplierName}</td><td>${r.branchName}</td>
      <td><span class="badge ${r.paymentStatus}">${r.paymentStatus}</span></td>
    </tr>`).join("");

    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/>
<title>Purchase Report</title><style>
...
<table><thead><tr>
  <th>#</th><th>PO No</th><th>Date</th><th>Year</th><th>Month</th>
  <th>Code</th><th>Item Name</th><th class="r">Qty</th><th>Unit</th>
  <th class="r">Unit Price</th><th class="r">Amount</th>
  <th>Supplier</th><th>Branch</th><th>Status</th>
</tr></thead><tbody>
${rowsHtml}
<tr class="tot">
  <td colspan="7">Total</td>
  <td class="r">${totals.qty}</td><td></td>
  <td></td>
...
export default function PurchaseReportPage() {
    ...
    // Raw rows from API (server only filters by date + branch + supplier)
    ...
    // productId → { brandId, subBrandId } built from purchase item payloads
    ...
    useEffect(() => {
        async function loadMasters() {
            ...
            try {
                const [b, br, sup] = await Promise.all([
    ...
    // ── API fetch (date + branch + supplier only — those are server-supported) ─
    const fetchReport = useCallback(async (f: Filters) => {
        ...
        try {
            ...
            const purchases: Purchase[] = result.purchases ?? [];
    ...
    // ── Client-side brand / sub-brand filter ────────────────────────────────────
            ...
            <div className="flex flex-col gap-2 md:flex-row md:items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-0.5">
                        Item-wise purchases with brand, sub-brand, supplier and date filters
                    </p>
                </div>
            ...
            <Card>
                <CardContent className="pt-4 pb-4">
                    <div className="flex flex-wrap gap-4 items-end">
                        ...
                        <FilterSelect
                            label={mastersLoading ? "Supplier (loading…)" : "Supplier"}
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b bg-muted/60 text-muted-foreground text-xs">
                                    ...
                                    <th className="px-3 py-2.5 text-left font-semibold">PO No</th>
                            ...
                            <tbody>
                                {rows.map((row) => (
                                    <tr key={row.slNo} className="border-b hover:bg-muted/30 transition-colors">
                                        ...
                                        <td className="px-3 py-2 font-mono text-xs">{row.poNo}</td>
                                        ...
                                        <td className="px-3 py-2 max-w-[130px] truncate" title={row.supplierName}>{row.supplierName}</td>
